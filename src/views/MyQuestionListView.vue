  ...
  <el-container :class="this.$store.getters.getDarkMode ? 'night-mode-chat-container': 'chat-container'" style="overflow: hidden">
    <el-header>
      <SearchBar
          ...
          :created-date-search="true"
          :updated-date-search="true"
          :admin="false"
      />
    </el-header>
    <el-main>
...
function doPageChangePost() {
  doPost("/questionList/queryUserQuestionList",{pageNo: storeQuestion.pageNo,
                                                        ...
                                                        createdAt: storeQuestion.createdDate,
                                                        updatedAt: storeQuestion.updatedDate
  }).then(response => {
    if(response && response.data.code === 200) {
